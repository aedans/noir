exports.card = {
	...
	colors: () => [],
	cost: () => ({ money: 30 }),
	play: (util, card) => () => card.modifiers.push({ name: "surveying", card: util.copy(card) }),
	update: {
		board: (util, card, player, opponent) => {
			card.numbers.opponentGuys = opponent.board

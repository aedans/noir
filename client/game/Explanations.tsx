...
export const explanationFontSize = 64;
...
export function ExplanationPopup(props: ExplanationProps) {
  ...
  function pointerdown() {
    ...
    anime({
      ...
      alpha: 0,
      complete: () => {
        props.setExplanations((es) => es.filter((e) => e.text != props.explanation.text));
      },
    });
  }

  const y = props.index * (explanationHeight - explanationBorder);

  return (
    <MoveAnimation id={props.explanation.id} x={0} y={y} scale={1} componentRef={ref}>
      <Container
